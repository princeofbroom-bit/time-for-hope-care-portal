...
import {
  ArrowLeft, Send, FileText, User, Mail, AlertCircle, CheckCircle
} from "lucide-react";
...
export default function NewSigningForm({ templates }: { templates: DocumentTemplate[] }) {
    ...
    try {
      ...
      const { error: insertError } = await supabase
        ...
        .insert({
          ...
          access_token: crypto.randomUUID() + '-' + Date.now().toString(36),
    ...
    <>
      ...
      <style jsx>{`
        ...
        .form-section h2 {
          ...
          font-size: 18px; margin-bottom: 20px; color: var(--primary-color);
        }
        .empty-text { color: #64748b; text-align: center; padding: 24px; }
        .empty-text a { color: var(--secondary-color); }
        .template-grid {
          display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 16px;
        }
        .template-card {
          position: relative; padding: 20px; border: 2px solid var(--border-color);
          border-radius: var(--radius-md); cursor: pointer; transition: all 0.2s;
        }
        .template-card:hover { border-color: var(--secondary-color); }
        .template-card.selected { border-color: var(--secondary-color); background: #f0fdf4; }
        .template-card input { position: absolute; opacity: 0; }
        .template-content h3 { font-size: 15px; margin-bottom: 6px; }
        .template-content p { font-size: 13px; color: #64748b; margin-bottom: 8px; }
        .template-type {
          font-size: 11px; padding: 3px 8px; background: #f1f5f9;
          border-radius: 10px; color: #64748b; text-transform: uppercase;
        }
        ...
        .form-group select {
          ...
          border-radius: var(--radius-md); font-size: 14px; transition: border-color 0.2s;
        }
        .form-group input:focus,
        .form-group select:focus { outline: none; border-color: var(--secondary-color); }
        ...
        .btn-primary {
          ...
          border-radius: var(--radius-md); font-weight: 600; cursor: pointer; transition: all 0.2s;
        }
        .btn-primary:hover:not(:disabled) { opacity: 0.9; }
        .btn-primary:disabled { opacity: 0.6; cursor: not-allowed; }
        .btn-secondary {
          padding: 12px 24px; background: white; color: var(--primary-color);
          border: 1px solid var(--border-color); border-radius: var(--radius-md);
          font-weight: 600; text-decoration: none; transition: all 0.2s;
        }
        .btn-secondary:hover { border-color: var(--primary-color); }
        .success-card {
          max-width: 500px; margin: 0 auto; text-align: center; padding: 48px;
          background: white; border-radius: var(--radius-lg); border: 1px solid var(--border-color);
        }
        .success-card h2 { margin: 20px 0 12px; color: #10b981; }
        .success-card p { color: #64748b; }
        .redirect-text { margin-top: 20px; font-size: 13px; }
