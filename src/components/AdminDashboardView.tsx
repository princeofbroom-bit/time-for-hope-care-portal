...
import {
  Users, ChevronRight,
  FileText, PenTool, UserPlus, Shield, Clock, CheckCircle
} from "lucide-react";
...
interface AdminDashboardProps {
  ...
  recentUsers: { id: string; name: string; role: string; updated_at: string }[];
}

export default function AdminDashboardView({
  totalUsers, pendingCount, completedCount, templateCount,
  workersCount, clientsCount, adminsCount, completionRate, recentUsers,
}: AdminDashboardProps) {
  return (
    <>
      ...
      <div className="analytics-row">
        <section className="dashboard-section chart-section">
          ...
          <div className="chart-container">
            <div className="donut-chart" style={{ '--percentage': `${(workersCount / Math.max(totalUsers, 1)) * 100}%` } as React.CSSProperties}>
              <div className="donut-center">
                <span className="donut-value">{totalUsers}</span>
          ...
          <div className="completion-chart">
            ...
            <div className="completion-stats">
              <div className="completion-stat">
                ...
                <span>{completedCount} Signed</span>
              </div>
              <div className="completion-stat">
      ...
      <style jsx>{`
        ...
        .stat-card h3 { font-size: 28px; margin-bottom: 2px; color: var(--primary-color); }
        .stat-card p { color: #64748b; font-size: 13px; margin: 0; }
        .dashboard-section { margin-bottom: 40px; }
        .dashboard-section h2 { font-size: 18px; margin-bottom: 20px; color: var(--primary-color); }
        .actions-grid {
        ...
        .action-card {
          ...
          border: 1px solid var(--border-color); display: flex; align-items: center;
          gap: 16px; text-decoration: none; transition: all 0.2s;
        }
        .action-card:hover {
          border-color: var(--secondary-color); transform: translateY(-2px); box-shadow: var(--shadow-md);
        }
        .action-content { flex: 1; }
        .action-content h3 { font-size: 15px; margin-bottom: 2px; color: var(--primary-color); }
        .action-content p { color: #64748b; font-size: 12px; margin: 0; }
        .analytics-row {
        ...
        .chart-section h2 { margin-bottom: 24px; }
        .chart-container { display: flex; align-items: center; gap: 32px; }
        .donut-chart {
        ...
        .donut-value { font-size: 24px; font-weight: 700; color: var(--primary-color); }
        ...
        .completion-chart { display: flex; flex-direction: column; align-items: center; gap: 20px; }
        .completion-ring { position: relative; width: 140px; height: 140px; }
        .completion-ring svg { width: 100%; height: 100%; }
        .completion-value {
          position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center;
        }
        .completion-value .percentage { display: block; font-size: 28px; font-weight: 700; color: var(--primary-color); }
        .completion-value .label { font-size: 11px; color: #64748b; }
        .completion-stats { display: flex; gap: 24px; }
        .completion-stat { display: flex; align-items: center; gap: 6px; font-size: 13px; color: #64748b; }
        .table-container {
          background: white; border-radius: var(--radius-lg);
          border: 1px solid var(--border-color); overflow: hidden;
        }
        .worker-table { width: 100%; border-collapse: collapse; text-align: left; }
        .worker-table th {
          padding: 14px 20px; background: #f8fafc; font-size: 12px; font-weight: 600;
          color: #64748b; text-transform: uppercase; letter-spacing: 0.5px;
          border-bottom: 1px solid var(--border-color);
        }
        .worker-table td { padding: 16px 20px; border-bottom: 1px solid var(--border-color); font-size: 14px; }
        ...
        .table-link {
          ...
          color: var(--secondary-color); font-weight: 600; font-size: 13px; text-decoration: none;
        }
